...
\begin{center}
{\bf STATISTICS 641 - ASSIGNMENT 6 - MIKE LEDERLE}
\end{center}
\vnn\vnn
...
\begin{shaded}
When $\{T_i\}_{i \leq n}$ are iid {\sl Exponential}($\beta$), then the distribution of 
$n\hat{\beta} = \sum T_i$  is {\sl Gamma}($n$, $\beta$). So here, $\beta$ = 5 and 
$n = 25$, so $\bar{Y} \sim  \frac{1}{n} \cdot {\sl Gamma}(25, 5)$.
\end{shaded}
\item[b.] What is the mean and standard deviation of $\bar Y$ if the exponential model is still valid?
\begin{shaded}
Mean of {\sl Gamma}(25, 5) = $n\beta =  5 \cdot 25 = 125$, so mean of 
$\bar{Y} = 125 / 25 = 5$ . Variance of {\sl Gamma}(25, 5) = $5 \cdot 25^2 = 3125$,  
so Var(1/25 * {\sl Gamma}(25, 5)) = 1/$25^2$ * Var({\sl Gamma}(25, 5)) = 3125 / 625 = 5,
so SD = 2.24.
\end{shaded}
\ee
...
<<>>=
beta <- 5
sim <- matrix(rexp(25 * 1000, rate = 1 / beta), ncol = 25)
sam.means <- apply(sim, 1, mean)
@
\end{shaded}
...
descdist(sam.means, boot = 1000)
fit <- fitdist(sam.means, "norm")
plot(fit)
gofstat(fit, print.test = TRUE)
@
Dr Longnecker mentioned the sensitivity of the GOF tests to large $n$, so the test rejections are not terribly worrisome. A fit to the normal seems a reasonable assumption.
\end{shaded}
...
\begin{shaded}
<<>>=
pgamma(5.2 * 25, shape = 25, rate = 1 / 5) - pgamma(4.8 * 25, shape = 25, rate = 1 / 5)
@
...
mean = 5, sd = 2.24.  
\begin{shaded}
Distributed (approx) as N(5, 5 / 25 = 1 / 5). So
<<>>=
pnorm(5.2, mean = 5, sd = 1 / sqrt(5)) - pnorm(4.8, mean = 5, sd = 1 / sqrt(5))
@
\end{shaded}
\item[c.] Using your simulated 1000 $\bar Y$s, approximate, $P[-0.2\le \bar Y - 5 \le 0.2]$.
\begin{shaded}
<<>>=
length(which(sam.means < 5.2 & sam.means > 4.8)) / length(sam.means)
@
\end{shaded}
\ee
\item[4.] Compare the three computations/estimators  of $P[-0.2\le \bar Y - 5 \le 0.2]$.
\begin{shaded}
The empirical smapling distribution estimate is close to the exact estimate, while the CLT is quite far away from the 
other two.  I think we need greater $n$ for CLT to be accurate.
\end{shaded}
\ee
\vnn
...
{\sl The MSE encapsulates both bias and variance.}
\end{shaded}
